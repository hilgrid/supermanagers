...
const STORAGE_KEY = 'session1-worksheet';
...
const Session1: React.FC = () => {
  ...
  // Worksheet state - initialize from localStorage
...
What I'm looking for when I evaluate this kind of work: ${whatImLookingFor}`;
  ...
  const HighlightedSpan = ({ text }: { text: string }) => (
    <span className="bg-rose-200 px-1">{text}</span>
  );

  const textareaClasses = "w-full mt-1 p-3 border border-stone-300 rounded-md text-stone-800 text-base leading-relaxed bg-white focus:outline-none focus:ring-2 focus:ring-stone-400 focus:border-transparent resize-y";
  ...
  return (
    <section className="relative min-h-screen w-full overflow-hidden" style={{ backgroundColor: '#faf8f5' }}>
      <div className="max-w-2xl mx-auto px-4 py-12">
        ...
        {/* Why Start Here */}
        <div className="mb-12">
          <h2 className="text-2xl font-bold text-stone-800 mb-4">Why start here?</h2>
          ...
          <p className="text-stone-800 text-base leading-relaxed mb-4">
            But the real reason we start here is that you're learning the skill underneath: how to define quality criteria and have AI evaluate against them. This is the foundation for everything else in this course. Automated workflows, multi-step tools, team-wide systems — none of it works if you can't define what "good" looks like at each step and write that into a prompt.
          </p>
          <p className="text-stone-800 text-base leading-relaxed mb-4">
          ...
          <p className="text-stone-800 text-base leading-relaxed">
            <a href="#bonus-challenges" className="underline underline-offset-2 hover:text-stone-600 transition-colors">Already comfortable making {setup.toolName}s? Try one of these bonus challenges.</a>
          </p>
        </div>
        ...
        <div className="mb-12">
          ...
          <p className="text-stone-800 text-sm mb-4">
            <a
              ...
              className="inline-flex items-center gap-1.5 font-bold text-stone-800 hover:text-stone-600 underline underline-offset-2 transition-colors"
            >
          ...
          <div className="bg-white border border-stone-300 rounded-lg p-6 mb-4">
            <div className="flex items-center justify-between mb-4">
              <p className="text-stone-400 text-sm">Your answers are saved in this browser. They'll be here when you come back. Filling this out on a different device than you'll use in class? Use the <a href="https://docs.google.com/document/d/1PLIJrhMTlSBw8TQ4AWYK5mw8898T9wizKQOB_-kymOY/edit?tab=t.0" target="_blank" rel="noopener noreferrer" className="underline hover:text-stone-500">Google Doc</a> instead.</p>
              <CopyButton getText={worksheetText} />
            </div>

            <label className="block mb-4">
              <span className="text-stone-800 text-base font-bold">The feedback I keep giving:</span>
              <span className="block text-stone-500 text-sm mt-0.5">What do you find yourself saying over and over? e.g., "Get to the point faster" or "This doesn't tell me what you want me to do"</span>
              <textarea
            ...
            <label className="block mb-4">
              <span className="text-stone-800 text-base font-bold">I give this feedback on:</span>
              <span className="block text-stone-500 text-sm mt-0.5">What kind of work? e.g., emails, presentations, reports, memos</span>
              <textarea
                className={textareaClasses}
                rows={1}
            ...
            <label className="block mb-4">
              <span className="text-stone-800 text-base font-bold">Who I give it to:</span>
              <span className="block text-stone-500 text-sm mt-0.5">Direct reports? Cross-functional partners? External stakeholders?</span>
              <textarea
                className={textareaClasses}
                rows={1}
                value={whoIGiveItTo}
            ...
            <label className="block mb-4">
              <span className="text-stone-800 text-base font-bold">They'd reach for this {setup.toolName} when:</span>
              <span className="block text-stone-500 text-sm mt-0.5">What's the specific trigger? e.g., "when they've finished a first draft" or "before sending an email to their skip level"</span>
              <textarea
                className={textareaClasses}
                rows={2}
            ...
            <label className="block mb-4">
              <span className="text-stone-800 text-base font-bold">The {setup.toolName} walks the user through these steps:</span>
              <span className="block text-stone-500 text-sm mt-0.5">Example: 1. User pastes their draft email. 2. {setup.toolName} checks it against the criteria. 3. {setup.toolName} gives feedback on each criterion. 4. {setup.toolName} suggests a revised version.</span>
              <textarea
                className={textareaClasses}
                rows={3}
            ...
            <label className="block">
              <span className="text-stone-800 text-base font-bold">What I'm looking for when I evaluate this kind of work:</span>
              <span className="block text-stone-500 text-sm mt-0.5">Just get it out of your head. What makes this good vs. bad? What do you notice when it's off? Doesn't need to be perfect or structured.</span>
              <textarea
                className={textareaClasses}
                rows={4}
        ...
        <div className="mb-12">
          ...
          <div className="bg-stone-100 border border-stone-300 rounded-lg p-4 mb-6">
            ...
            <p className="text-stone-500 text-sm mt-2">You'll be switching between these tabs throughout the session. Keep all three open once you have them.</p>
          </div>
        </div>

        {/* Part 1 */}
        <div className="mb-12">
          <h3 className="text-xl font-bold text-stone-800 mb-3">Part 1: Demo — Building a {setup.toolName}</h3>
          <p className="text-stone-500 text-sm font-medium mb-4">10 min</p>
          <p className="text-stone-800 text-base leading-relaxed italic mb-4">
          ...
          <details className="mb-4 border border-stone-300 rounded-lg overflow-hidden">
            ...
            <div className="px-4 py-4">
              <div className="bg-stone-50 rounded p-4 text-stone-800 text-sm leading-relaxed font-mono">
                <p>"Can you write a prompt for a {setup.toolName} that gives feedback on emails to execs?"</p>
              </div>
              <p className="text-stone-600 text-sm mt-3">That's it. No context about what kind of feedback, who it's for, what good looks like. Watch what happens.</p>
            </div>
          </details>

          <div className="bg-stone-100 border-l-4 border-stone-400 p-4">
            <p className="text-stone-800 text-base leading-relaxed">
              <strong>Key takeaway:</strong> Aligning with the AI on what you want and how it should work <em>before</em> it starts writing gets you a dramatically better result.            </p>
          </div>
        </div>
        ...
        <div className="mb-12">
          ...
          <details className="mb-4 border border-stone-300 rounded-lg overflow-hidden">
            ...
            <div className="px-4 py-4">
              ...
              <p className="text-stone-600 text-sm mt-3">Notice how much more specific this is than the template above. The CEO preference for alternatives, the common failure modes, pass/fail instead of scored - these details are what turn generic feedback into feedback that sounds like you.</p>
            </div>
          </details>
          ...
          <details className="mb-4 border border-stone-300 rounded-lg overflow-hidden">
            ...
            <div className="px-4 py-4 space-y-4">
              <div>
                <p className="text-stone-500 text-xs font-medium uppercase tracking-wide mb-2">Example 1</p>
                <div className="bg-stone-50 rounded p-4 text-stone-800 text-sm leading-relaxed font-mono">
              ...
              <div>
                <p className="text-stone-500 text-xs font-medium uppercase tracking-wide mb-2">Example 2</p>
                <div className="bg-stone-50 rounded p-4 text-stone-800 text-sm leading-relaxed font-mono">
          ...
          <details className="mb-4 border border-stone-300 rounded-lg overflow-hidden">
            ...
            <div className="px-4 py-4 space-y-4">
              <p className="text-stone-600 text-sm">One strong email and one that's not as sharp. The AI should be able to tell the difference.</p>
              <div>
                <p className="text-stone-500 text-xs font-medium uppercase tracking-wide mb-2">Example 1 (Good)</p>
                <div className="bg-stone-50 rounded p-4 text-stone-800 text-sm leading-relaxed font-mono">
              ...
              <div>
                <p className="text-stone-500 text-xs font-medium uppercase tracking-wide mb-2">Example 2 (Mediocre)</p>
                <div className="bg-stone-50 rounded p-4 text-stone-800 text-sm leading-relaxed font-mono">
          ...
          <details className="mb-6 border border-stone-300 rounded-lg overflow-hidden">
            ...
            <div className="px-4 py-4">
              ...
              <p className="text-stone-600 text-sm mt-3">This is a good instinct for any feedback tool. Think about whether yours should teach or just fix.</p>
            </div>
          </details>

          <h4 className="text-lg font-bold text-stone-800 mt-8 mb-2">Step 4: Set up your {setup.name}</h4>
          ...
          <h4 className="text-lg font-bold text-stone-800 mt-8 mb-2">Step 5: Test and refine</h4>
          <p className="text-stone-800 text-base leading-relaxed mb-3">
          ...
          <div className="bg-stone-100 border border-stone-300 rounded-lg p-4 mb-4">
            <ol className="text-stone-800 text-sm leading-relaxed space-y-1 list-decimal list-inside">
              <li><strong>Your {setup.toolName}</strong> - where you test it by pasting in real work</li>
              <li><strong>The prompt-building conversation</strong> - where you tell the AI what to change and get an updated prompt</li>
              <li><strong>The {setup.toolName} editor</strong> - where you paste the updated prompt into the instructions</li>
            </ol>
            <p className="text-stone-500 text-sm mt-2">The loop is: test in tab 1, refine in tab 2, paste the new prompt in tab 3, then test again in tab 1.</p>
          </div>
          <ol className="text-stone-800 text-base leading-relaxed space-y-3 mb-4 list-decimal list-inside">
            <li><strong>Open your {setup.toolName} in a new window.</strong> Keep the prompt-building conversation open in a separate tab.</li>
            <li><strong>Paste in a real email</strong> (or use the sample below) and send it.</li>
            <li><strong>Read the feedback.</strong> Do you agree with it? Would you have said the same things? If it says "great job!" on something you'd send back, that's a problem.</li>
            <li><strong>Go back to your prompt-building conversation.</strong> Tell it what you didn't like about the feedback - what it missed, what it got wrong, what felt off. Ask it what changes it would recommend making to the prompt.</li>
            <li><strong>If you agree with the changes, tell it to make them.</strong> Copy the updated prompt.</li>
            <li><strong>Paste the new prompt back into your {setup.toolName}'s instructions</strong> (replacing the old one) and test again.</li>
          </ol>
          <p className="text-stone-800 text-base leading-relaxed mb-3">
        ...
        <div className="mb-12">
          ...
          <p className="text-stone-500 text-sm font-medium mb-4">10 min</p>
          <p className="text-stone-800 text-base leading-relaxed">
            Everyone built the same type of {setup.toolName}, but with different criteria and context. We'll hear from a few people: what criteria did you use? What did the feedback look like? Did anything surprise you?          </p>
        </div>
        ...
        <div className="mb-12">
          ...
          <p className="text-stone-500 text-sm mb-6">
            If you'd prefer to finish this on your own time, this guide is designed to work asynchronously. The main thing you'll miss is being able to ask questions in real time - so save those for the Maven portal or email.
          </p>

          <h4 className="text-lg font-bold text-stone-800 mb-2">Step 1: Generate your prompt</h4>
          <p className="text-stone-800 text-base leading-relaxed mb-3">
          ...
          <div className="mb-6">
            <button
              ...
              className="px-5 py-2.5 text-sm font-medium rounded-md transition-colors bg-stone-800 text-white hover:bg-stone-700"
            >
              Generate prompt from my worksheet
            </button>

            {generatedPrompt ? (
              <div ref={generatedPromptRef} className="mt-4 bg-rose-50 border-l-4 border-rose-300 p-4">
                <div className="flex justify-end mb-2">
          ...
          <ol className="text-stone-800 text-base leading-relaxed space-y-3 mb-4 list-decimal list-inside">
            <li><strong>Open your {setup.toolName} in a new window.</strong> Keep the prompt-building conversation open in a separate tab.</li>
            <li><strong>Paste in a real example</strong> of the kind of work your {setup.toolName} is supposed to evaluate.</li>
            <li><strong>Read the feedback.</strong> Do you agree with it? What did it miss? What felt off?</li>
            <li><strong>Go back to your prompt-building conversation.</strong> Tell it what you didn't like and ask it what changes it would recommend to the prompt.</li>
            <li><strong>If you agree with the changes, tell it to make them.</strong> Copy the updated prompt.</li>
            <li><strong>Paste the new prompt back into your {setup.toolName}'s instructions</strong> and test again.</li>
          </ol>
          <p className="text-stone-800 text-base leading-relaxed mb-3">
        ...
        <div className="mb-12">
          ...
          <h4 className="text-base font-bold text-stone-800 mb-2">What if I don't have examples of what good looks like?</h4>
          <p className="text-stone-800 text-base leading-relaxed mb-3">
            A few options, all with trade-offs:
          </p>
          <ul className="text-stone-800 text-base leading-relaxed space-y-2 mb-3 list-disc list-inside">
            <li><strong>Just explain it.</strong> Write down what you'd say if you were giving the feedback live. That's often enough to get started.</li>
            <li><strong>Search for examples online.</strong> If you're building something around keynote slides or executive memos, there are good examples out there.</li>
            <li><strong>Ask the AI to generate examples.</strong> This is called synthetic data, and it can work, especially for text-based outputs. Just know that the AI will generate "generically good" examples based on broad best practices.</li>
          </ul>
          <p className="text-stone-800 text-base leading-relaxed mb-6">
          ...
          <h4 className="text-base font-bold text-stone-800 mb-2">What's Markdown? Do I need to learn it?</h4>
          <p className="text-stone-800 text-base leading-relaxed mb-3">
            You'll notice the system prompt uses <code className="bg-stone-100 px-1 rounded text-sm">#</code> for headings and <code className="bg-stone-100 px-1 rounded text-sm">**</code> for bold text. That's Markdown - a simple formatting style that helps AI understand structure, similar to how different header sizes in a Word doc help you scan a page.
          </p>
          <p className="text-stone-800 text-base leading-relaxed mb-6">
            You don't need to learn it. When the AI generates a system prompt, it'll already be in Markdown. If you're curious, the basics are: <code className="bg-stone-100 px-1 rounded text-sm">#</code> for a main heading, <code className="bg-stone-100 px-1 rounded text-sm">##</code> for a subheading, <code className="bg-stone-100 px-1 rounded text-sm">**bold**</code> for bold, and <code className="bg-stone-100 px-1 rounded text-sm">-</code> for bullet points. If you ever want to convert a Google Doc to Markdown, you can download it as .md or just paste the content into the AI and ask it to convert it.
          </p>
        ...
        {/* Advanced */}
        <details id="bonus-challenges" className="mb-16 border border-stone-300 rounded-lg overflow-hidden">
          <summary className="px-6 py-4 bg-stone-50 text-stone-800 font-bold cursor-pointer hover:bg-stone-100 transition-colors">
            Already comfortable building {setup.toolName}s? Try one of these instead.
          </summary>
          <div className="px-6 py-6 space-y-6">
            <div>
              <h4 className="text-base font-bold text-stone-800 mb-2">Challenge 1: Build a teaching tool, not a fixing tool</h4>
              <p className="text-stone-800 text-base leading-relaxed">
                Most feedback tools just tell people what to change. A better version explains <em>why</em> something isn't working and shows what a stronger version would look like - so the person learns to catch it themselves next time. Redesign your {setup.toolName}'s prompt so that if someone used it for six months and then lost access, they'd still be better at the task than when they started. <a href="https://leaf-balance-3b8.notion.site/Supermanagers-Prompt-Library-2026-2f1fac122bde8021a541cb453d50d82d" target="_blank" rel="noopener noreferrer" className="underline underline-offset-2 hover:text-stone-600 transition-colors">See examples under section 2.2.</a>
              </p>
            </div>
            <div>
              <h4 className="text-base font-bold text-stone-800 mb-2">Challenge 2: Build a multi-step workflow</h4>
              <p className="text-stone-800 text-base leading-relaxed">
                Instead of evaluating a single input, design a {setup.toolName} that walks someone through a process - step by step, with the AI guiding them through each stage. Think about a task where people usually skip steps or do them out of order. What would it look like if the AI helped them get it right from the start? <a href="https://leaf-balance-3b8.notion.site/Supermanagers-Prompt-Library-2026-2f1fac122bde8021a541cb453d50d82d" target="_blank" rel="noopener noreferrer" className="underline underline-offset-2 hover:text-stone-600 transition-colors">See examples under section 2.2.</a>
              </p>
            </div>
          </div>
        </details>
